...
const carIndex = inventory.findIndex(x => x.id === 33);
console.log(`Car 33 is a ${inventory[carIndex].car_year}, ${inventory[carIndex].car_make}, ${inventory[carIndex].car_model}` );
...
let carModels = [];
carModels = inventory.sort((a, b) => a.car_model.charAt().toLowerCase() > b.car_model.charAt().toLowerCase());
...
let oldCars =[];
const carYearsCopy = [...carYears];
for (let i = 0; i < carYearsCopy.length; i++) {
    if (carYearsCopy[i] < 2000) {
        oldCars.push(carYearsCopy[i]);
    }
}
